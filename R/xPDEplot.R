PDEplot <- function(data,paretoRadius=0,weight=1,kernels=NULL,LogPlot=F,PlotIt=TRUE,title="ParetoDensityEstimation(PDE)",color="blue",xpoints=FALSE, xlim,ylim,xlab ='Data',ylab ='PDE',ggPlot=ggplot(),sampleSize=200000)
{
...
# 3.Editor: MT: Fehlerabfang fuer ggobject, sampelsize
  # Daten normalisieren
  if(!is.vector(data)){
    warning('data is not a vector, as.vector() is called')
    data=as.vector(data)
  }
  if(!is.numeric(data)){
    warning('data is not numeric, as.numeric() is called')
  ...
  if(xpoints){
    plt <- plt + geom_point(data = df, aes(x = kernels, y = density), colour = color)
  }else{
    plt <- plt + geom_line(data = df, aes(x = kernels, y = density), colour = color)  
  }
